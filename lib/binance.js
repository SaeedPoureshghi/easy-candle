...
export async function fetchCandlesPage(params) {
  ...
  if (!response.ok) {
    // Prefer the proxy's short client message; keep detail for non-rate-limit errors.
    if (response.status === 429 || response.status === 503) {
      throw new Error(body.error || "Binance temporarily unavailable. Try again shortly.");
    }
